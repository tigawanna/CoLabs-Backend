...
import { and, asc, desc, eq, getTableName, sql } from "drizzle-orm";
...
export class AuditlogsService {
  ...
  async findAll(query: z.infer<typeof listAuditlogsQueryParamsSchema>) {
    const { search, action, table, ...paginationQuery } = query;
    const conditions = and(
      // search ? ilike(auditLogsTable.table, `%${search}%`) : undefined,
      table ? eq(auditLogsTable.table, table) : undefined,
