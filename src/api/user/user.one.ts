...
export const userGetOneRoute = createRoute({
  path: "/{id}",
  method: "get",
  tags,
  request: {
...
export type GetOneRoute = typeof userGetOneRoute;

const userService = new UserService();
export const userGetOneHandler: AppRouteHandler <GetOneRoute> = async (c) => {
  try {
    const oneItem = await userService.findById(c.req.valid("param").id);
    if (!oneItem) {
      return c.json({
        result: null,
        error: {
